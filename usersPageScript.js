jQuery(document).ready(function($) {
	var friend = $("#my_id").val();
$.ajax({
	url :"server.php",
	type: "post",
	data: {action: "friendsPage", friend },
	success: function(r){
		r = JSON.parse(r);
		r.forEach(function(item){

			$(".div_anun").append(`<span> ${item.name + " " + item.surname}</span>`);
			$(".span_picture").append(`<img alt="Image placeholder" src="${item.profil}">`); 
			$(".p_anun").after(`<h1 class="display-2 text-white">Hello ${item.name}</h1>`);
			$(".a_picture").append(`<img id="glavPhoto" src= "${item.profil}" class="rounded-circle"> `);
			$(".h3_hihi").append(`${item.name + " " + item.surname}<span class="font-weight-light">, ${item.age}</span>`);
		})
	}
})
...
$.ajax({
	url: "server.php",
	type: "post",
	data: {friend, action: "getPostsUser"},
	success: function(r){
		r = JSON.parse(r);
		console.log(r)
		r.forEach(function(item){
